...
- 管理者機能は非公開の管理者専用URLから提供する。通常のユーザー導線からはリンクしない。
  ...
  認証方式は **推測不可能な秘密URLのみ**（ログインフォーム・パスワード・セッションは持たない）。
  `/admin/[token]` の `token` を環境変数 `ADMIN_SECRET` と一致するかだけで判定し、不一致なら`notFound()`で
  「そもそもページが存在しない」ように見せる（403ではなく404を返す）。本番(Vercel)にも`ADMIN_SECRET`の設定が必須。
